...
(function($) {
    ...
    function seekPage(e, open) {
        ...
        $this.find('.mf-container').stop();

        // Reset open page
        if (openId > 0) {
            var $openElem = pages[openId - 1];
            $openElem.toggleClass('mf-active');
            $openElem.removeClass('mf-large').find('.mf-full').hide().empty();
            $openElem.animate({
                width: $plugin.settings.pageWidth
            }, $plugin.settings.pageCloseDuration, $plugin.settings.pageCloseEasing, function() {
                $openElem.find('.mf-preview').fadeIn();
            });
            openId = 0;
        }

        $this.find('.mf-container').animate({
            left: -(displayOffset) * ($plugin.settings.pageWidth + $plugin.settings.pageSeparation)
        }, $plugin.settings.pageSeekDuration, $plugin.settings.pageSeekEasing, function() {
            if (open == true) {
                openId = activeId;

                var $activeElem = pages[activeId - 1];
                $activeElem.find('.mf-preview').hide();
                $.ajax({
                    dataType: 'html',
                    beforeSend: function() {
                        $activeElem.addClass('loading');
                    },
                    error: function() {
                    ...
                    success: function(data) {
                        ...
                        $('<iframe scrolling="no" />').load(function() {
                            var iframe = $activeElem.find('.mf-full iframe');
                        ...
                        }).appendTo($activeElem.find('.mf-full'));
                        ...
                        $activeElem
                            ...
                            .addClass('mf-large')
                            .animate({
                                width: pageWidthLarge
                            }, $plugin.settings.pageOpenDuration, $plugin.settings.pageOpenEasing, function() {
                                $activeElem.removeClass('loading');
                                $activeElem.find('.mf-full').fadeIn();
                            });
                    },
                    data: {
                        xmlhttp: true
                    },
                    url: $activeElem.find($plugin.settings.permaLinkSelector).attr('href')
                });
            }
        });
    }
    ...
    var methods = {
        init: function(options) {
            ...
            return this.each(function() {
                ...
                if (isNaN($plugin.settings.pageWidthLarge)) {
                    $plugin.settings.pageWidthLarge = $this.innerWidth();
                }
                ...
                $this.find($plugin.settings.pageSelector).each(function() {
                    ...
                    if (openId == 0) {
                        ...
                        if ($elem.hasClass('mf-active')) activeId = openId = _index;
    ...
    $.fn[plugin].defaults = {
        pageSelector: '.mf-page',
        filterSelector: '.mf-filters',
        initialFilter: 'all',
        pageWidth: 300,
        pageSeparation: 10,
        pageWidthLarge: 'auto',
        tocLinkSelector: '.mf-toc',
        permaLinkSelector: '.mf-permalink',
        ...
        navNextSelector: '.mf-next',
        navPreviousSelector: '.mf-previous',
        closeLinkSelector: '.mf-close',
        useMouseWheel: true
    };
